...
main(void)
{
	...
	#endif
	//printf("%s\n", msg);
	//printf("%f",deviceProp.totalGlobalMem);
	totalmem=deviceProp.totalGlobalMem;
	}
	printf("\n%f\n",totalmem);
	float memperarr=deviceCount*totalmem/12;
    ...
    if((float)numElements>memperarr){
    	printf("Size of array exceeds total device memory!\nMaximal number of elements in array is %d",(int)memperarr);
		//exit(EXIT_FAILURE);
